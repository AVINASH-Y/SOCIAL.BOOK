...
function App() {
  ...
  const handleChange = (event) => {
    ...
    if (name.startsWith('education[') || name.startsWith('experience[') || name.startsWith('projects[') || name.startsWith('awards[')) {
      const [field, index, subfield] = name.match(/\w+/g);
      setFormData((prevFormData) => ({
        ...prevFormData,
        [field]: prevFormData[field].map((item, idx) =>
          idx === Number(index) ? { ...item, [subfield]: value } : item
        ),
      }));
    } else {
      setFormData({
